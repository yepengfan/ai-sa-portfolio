...
```
No caching: $0.0245  (3,049 input tokens at standard rate)
Cold call:  $0.0259  (1,178 regular + 1,871 cache write at 1.25x)
Warm call:  $0.0195  (1,178 regular + 1,871 cache read at 0.1x)
```

**Result: 20.6% saving per warm call. Breaks even after just 2 calls** — the cache write premium ($0.0014) is recouped on the very first warm call.

At 1,000 calls/day: **$5.00/day → $150/month saved** from adding one field to the API request.
...
| **Prompt Caching** | 20.6% | None | Minimal | Repeated system prompt |
...
The combined theoretical saving: caching (20.6%) + routing on simple queries (35.1%) + compression on long contexts (76.3%) — though the actual combined saving depends on workload mix and isn't simply additive.

## Practical Recommendations

**Start here (5 minutes):** Add `cache_control: {"type": "ephemeral"}` to your system prompt. Instant ~20% saving on repeated prompts with zero quality impact.
...
1. LLMLingua-2: Data Distillation for Efficient and Faithful Task-Agnostic Prompt Compression — [Microsoft Research](https://www.microsoft.com/en-us/research/publication/llmlingua-2-data-distillation-for-efficient-and-faithful-task-agnostic-prompt-compression/)
2. LLMLingua Project & Source Code — [GitHub (microsoft/LLMLingua)](https://github.com/microsoft/LLMLingua)
3. Amazon Bedrock Prompt Caching — [AWS Documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/prompt-caching.html)
4. Amazon Bedrock Pricing (Claude Sonnet, Haiku, cache read/write, batch) — [AWS Bedrock Pricing](https://aws.amazon.com/bedrock/pricing/)
5. Amazon Bedrock Batch Inference — [AWS Documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/batch-inference.html)
6. LongBench: A Bilingual, Multitask Benchmark for Long Context Understanding — [GitHub (THUDM/LongBench)](https://github.com/THUDM/LongBench)
